...
import { ChevronLeft, ChevronRight, Music, Share2 } from "lucide-react";
import { Playlist as ApiPlaylist } from "@/api/playlists";
import { PlaylistCard } from "../ui/PlaylistCard";

interface SharedPlaylistsSectionProps {
  playlists?: ApiPlaylist[];
  isLoading?: boolean;
}

export const SharedPlaylistsSection: React.FC<SharedPlaylistsSectionProps> = ({
  playlists = [],
  isLoading = false,
}) => {
      ...
      {displayPlaylists.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center pt-12 text-center">
          <div className="rounded-full bg-muted/40 p-4 mb-4">
            <Share2 size={32} className="text-muted-foreground" />
          </div>
          <h3 className="text-lg font-semibold text-foreground">
            No shared playlists
          </h3>
          <p className="mt-2 text-sm text-muted-foreground">
            Playlists shared by others will appear here
          </p>
        </div>
      )}
